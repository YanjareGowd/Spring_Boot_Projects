...
public interface QuestionDao extends JpaRepository<Question, Integer> {
	...
	//@Query(value = "SELECT * FROM question q where q.category ORDER BY RA")
...
//	List<Question> findRandomQuestionByCategory(@Param ("category") String category, @Param("numQ") int numQ);
	@Query(value = "SELECT * FROM quizsetquestion q WHERE q.category=:category ORDER BY RAND() LIMIT :numQ", nativeQuery = true)
	List<Question> findRandomQuestionByCategory(@Param("category") String category, @Param("numQ") int numQ);
